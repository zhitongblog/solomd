...
export const t = {
  en: {
    ...
    hero: {
      ...
      lead: 'Lightweight Markdown editor for macOS / Windows / Linux + a bundled MCP server that exposes the same vault to any LLM client. 14 AI providers BYOK, GitHub-backed E2EE sync, AutoGit history per note. ~15 MB. No subscription. No SoloMD server.',
      ...
      ctaWhatsNew: "What's new in v3.0 →",
      chips: ['🔌 MCP server bundled', '🤖 14 AI providers · BYOK', '☁️ GitHub-backed sync', '🔒 E2EE on your repo', '📜 AutoGit per note', '🌏 中文 first-class'],
      demoAlt: 'SoloMD editor demo — live preview, split view, and slideshow mode',
    },
    features: {
      heading1: 'A folder of markdown.',
      heading2: 'Plus the surface to drive it.',
      lead: 'Two halves of one product: an editor that writes plain `.md` files to disk, and a bundled MCP server (`solomd-mcp`) that lets any LLM client — Claude Code, Cursor, Codex — read and edit the same vault from outside. Same notes. Two ways in.',
      items: [
  ...
  zh: {
    ...
    hero: {
      badge: 'v3.0 · MIT 协议 · 跨平台',
      title1: '一个 Markdown 编辑器,',
      title2: '也是连接 LLM 的桥。',
      subtagline: '你的笔记是一个文件夹。SoloMD 既是上面的编辑器,也是 Claude Code / Codex / Cursor 直接能接的 MCP 端点。',
      lead: '轻量 Markdown 编辑器,支持 macOS / Windows / Linux,内置 MCP server 把同一个 vault 暴露给任意 LLM 客户端。14 个 AI 服务 BYOK,GitHub 端到端加密同步,每篇笔记自带 AutoGit 历史。约 15 MB,无订阅,无 SoloMD 服务器。',
      ...
      ctaWhatsNew: 'v3.0 新功能 →',
      chips: ['🔌 自带 MCP server', '🤖 14 个 AI · BYOK', '☁️ GitHub 同步', '🔒 端到端加密在你自己的库', '📜 每笔记 AutoGit', '🌏 中文一等公民'],
      demoAlt: 'SoloMD 演示 — 实时预览、分屏、演讲模式',
    },
    features: {
      heading1: '一个 Markdown 文件夹,',
      heading2: '加上能驱动它的入口。',
      lead: '同一份产品的两半:把纯 `.md` 写到磁盘上的编辑器,加上把同一个 vault 暴露给任意 LLM 客户端 (Claude Code / Cursor / Codex) 的 `solomd-mcp` 服务。同样的笔记,两条入口。',
      items: [
