...
const visibleRsPanes = computed(() => {
  ...
  const all: Record<'search' | 'outline' | 'backlinks' | 'relationships' | 'tags' | 'history' | 'agent', boolean> = {
  ...
  const known = ['search', 'outline', 'backlinks', 'relationships', 'tags', 'history', 'agent'] as const;
  ...
  return ordered
    ...
    .map((id) => ({ id: id as 'search' | 'outline' | 'backlinks' | 'relationships' | 'tags' | 'history' | 'agent' }));
});
