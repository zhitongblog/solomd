...
## 7. Comparative Audit: Legacy vs Unified Pipeline

The unified `.github/workflows/release.yml` fully supersedes the legacy implementation across all operational vectors. The legacy workflow (`release.legacy.disabled`) is deprecated and decommissioned.

### Architecture Comparison Matrix

| Capability | Legacy (`release.legacy.disabled`) | Unified (`.github/workflows/release.yml`) |
| :--- | :--- | :--- |
| **macOS Universal (`.dmg`)** | ❌ Omitted (required local manual build) | ✅ Fully automated CI pipeline (`aarch64` + `x86_64`) |
| **Windows (`x64` / `ARM64`)** | ⚠️ MSI + Portable zip | ✅ MSI + Portable zip + Standalone MCP + Defender Gate |
| **Linux (`x64` / `ARM64`)** | ⚠️ `ubuntu-22.04` (deprecated runtime) | ✅ `ubuntu-24.04` (`.AppImage`, `.deb`, `.rpm`, `.tar.gz`) |
| **Satellites** | ❌ Omitted from release lifecycle | ✅ Parallel WebExtension (Chrome/Firefox) + Agent Skill Pack |
| **DAG Topology** | ❌ Monolithic race condition on draft release | ✅ 4-Phase Symmetric DAG (`Satellites` ➔ `Desktop` ➔ `Release & Provenance` ➔ `Distribution`) |
| **Single Source of Truth** | ❌ Fragmented bash scripting | ✅ Deterministic `dist-manifest.json` metadata engine |
| **Supply-Chain Security** | ❌ None | ✅ CycloneDX JSON SBOM + SLSA Level 3 Provenance (Sigstore) |
| **Homebrew Cask Tap** | ❌ Manual script execution | ✅ Automated dynamic push to `zx0r/homebrew-solomd` |
| **Crates.io Publishing** | ❌ Manual static token publish | ✅ RFC 8693 OIDC Trusted Publishing for `solomd-mcp` |
